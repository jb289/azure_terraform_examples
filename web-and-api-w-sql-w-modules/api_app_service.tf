...
resource "null_resource" "webhook_api_example" {
  # Changes to the ids will cause this to re-provision
  triggers = {
    app_instance_ids = "${module.api_app_service_1.app_service.id},${module.api_app_service_2.app_service.id}"
  }

  provisioner "local-exec" {
    command = "echo ********* LOCAL-EXEC - WEB APP 1 ${module.api_app_service_1.app_service.default_site_hostname} *********"
  }

  provisioner "local-exec" {
    command = "echo ********* LOCAL-EXEC - WEB APP 2 ${module.api_app_service_2.app_service.default_site_hostname} *********"
  }
